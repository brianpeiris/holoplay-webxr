                ...
                class HoloPlayXRDevice extends XRDevice {
                  ...
                  onFrameStart(sessionId, renderState) {
                    ...
                    if (session.immersive) {
                      ...
                      const baseLayerPrivate = session.baseLayer[PRIVATE$j];
                      baseLayerPrivate.clearFramebuffer();
                    } else {
                      const gl = session.baseLayer.context;
                      const aspect = gl.drawingBufferWidth / gl.drawingBufferHeight;
                      perspective$1(this.inlineProjectionMatrix, renderState.inlineVerticalFieldOfView, aspect,
                        renderState.depthNear, renderState.depthFar);
                      fromTranslation(this.basePoseMatrix, [0, kDefaultEyeHeight, 0]);
                      invert$2(this.inlineInverseViewMatrix, this.basePoseMatrix);
                    }
                  }
                  onFrameEnd(sessionId) {
                    const session = this.sessions.get(sessionId);
                    session.baseLayer[PRIVATE$j].blitTextureToDefaultFramebufferIfNeeded();
                  }
                  async requestFrameOfReferenceTransform(type, options) {
                  ...
                  endSession(sessionId) {
                    const session = this.sessions.get(sessionId);
                    if (session.immersive && session.baseLayer) {
                      session.baseLayer[PRIVATE$j].moveCanvasToWindow(false);
